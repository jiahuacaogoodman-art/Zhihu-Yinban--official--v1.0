...
async function onPatientChange() {
  if (!selectedPid.value) { memories.value = []; return }
  try {
...
async function recordOutcome(status: string) {
  ...
  try {
    await api.patch(`/nursing/decisions/${encodeURIComponent(currentDecisionId.value)}/outcome`, {
      outcome_status: status, note: `管理端标记: ${status}`, recorded_by: '管理端',
    })
    toast({ tone: 'success', text: '结果已记录' })
    onPatientChange()
  } catch (e: any) { toast({ tone: 'error', text: e.message ?? '记录失败' }) }
}
...
<template>
  <div class="nd-view">
    <GlassPanel variant="card">
      ...
      <div class="nd-patient-row">
        ...
        <select v-model="selectedPid" class="field nd-select" @change="onPatientChange">
          ...
          <option v-for="p in filteredPatients()" :key="p.patient_id" :value="p.patient_id">
      ...
      <div class="nd-chips">
        <button v-for="s in quickSymptoms" :key="s" class="tap-chip" @click="addSymptom(s)">{{ s }}</button>
      </div>

      <Field v-model="symptomText" type="textarea" :rows="3" placeholder="描述老人目前的症状或护理需求…" />

      <div class="nd-submit">
        <Btn variant="primary" :loading="aiLoading" @click="submitDecision">获取 AI 护理建议</Btn>
      </div>
    </GlassPanel>

    <GlassPanel v-if="adviceText" class="nd-advice">
      <template #header><span class="title-s">AI 护理建议</span></template>
      ...
      <div class="nd-disclaimer">⚠️ AI 生成内容仅供参考。持续不适或出现严重症状，请立即联系医生或启动急救流程。</div>
    </GlassPanel>
    ...
    <GlassPanel v-if="memories.length > 0">
      <template #header><span class="title-s">近期决策记忆</span></template>
      <div class="nd-memories">
        <div v-for="m in memories" :key="m.decision_id" class="nd-mem-item">
          ...
          <div class="meta">
            {{ m.created_at }}
            <Chip v-if="m.outcome_status" :tone="m.outcome_status === 'effective' ? 'success' : m.outcome_status === 'ineffective' ? 'danger' : 'warning'" style="margin-left: 6px;">{{ m.outcome_status }}</Chip>
          </div>
        </div>
      </div>
    </GlassPanel>
...
<style scoped>
.nd-view { display: grid; gap: var(--sp-4, 16px); max-width: 900px; }
.nd-patient-row { display: grid; grid-template-columns: 1fr 2fr; gap: var(--sp-2, 8px); margin-bottom: var(--sp-3, 12px); }
.nd-select { height: 40px; }
.nd-chips { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: var(--sp-3, 12px); }
.nd-submit { margin-top: var(--sp-3, 12px); }
.nd-advice { border: 1px solid rgba(20, 184, 166, 0.2); }
.nd-outcome { display: flex; align-items: center; gap: 8px; margin-top: var(--sp-3, 12px); padding-top: var(--sp-2, 8px); border-top: 1px solid rgba(15, 23, 42, 0.06); }
.nd-disclaimer { margin-top: var(--sp-3, 12px); padding: var(--sp-2, 8px) var(--sp-3, 12px); border-radius: var(--r-s, 10px); background: rgba(217, 119, 6, 0.08); color: var(--amber, #d97706); font: 400 var(--fz-xs, 11px) / 1.5 var(--font-ui); }
.nd-evidence { display: grid; gap: 8px; }
.nd-ev-item { display: flex; align-items: flex-start; gap: 8px; padding: 8px; border-radius: 8px; background: rgba(15, 23, 42, 0.02); }
.nd-memories { display: grid; gap: 8px; }
.nd-mem-item { padding: 8px 12px; border-radius: 8px; background: rgba(15, 23, 42, 0.02); border-left: 3px solid var(--accent, #14b8a6); }
@media (max-width: 640px) { .nd-patient-row { grid-template-columns: 1fr; } }
</style>
