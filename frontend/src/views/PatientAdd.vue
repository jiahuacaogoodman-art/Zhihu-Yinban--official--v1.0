...
const form = ref({
  ...
  care_level: '', bed_number: '', primary_nurse: '', admission_date: '',
  emergency_contact: '', emergency_phone: '', emergency_relation: '',
  medical_history: '', allergy: '', diet_restriction: '', notes: '',
})

async function submitPatient() {
  if (!form.value.patient_id.trim() || !form.value.name.trim()) {
    toast({ tone: 'warning', text: '编号和姓名为必填项' }); return
  }
  loading.value = true
  try {
  ...
  } catch (e: any) { toast({ tone: 'error', text: e.message ?? '保存失败' }) }
  ...
  <div class="pa-view">
    <GlassPanel variant="card">
      ...
      <div class="form-grid cols-3">
        ...
        <div class="field-group"><span class="field-label">性别</span>
          <select v-model="form.gender" class="field"><option value="">请选择</option><option>男</option><option>女</option><option>其他</option></select>
        </div>
      ...
      <div class="form-grid cols-3">
        <div class="field-group"><span class="field-label">血型</span>
          <select v-model="form.blood_type" class="field"><option value="">—</option><option>A</option><option>B</option><option>AB</option><option>O</option></select>
        </div>
      ...
      <div class="form-grid cols-4">
        <div class="field-group"><span class="field-label">护理级别</span>
          <select v-model="form.care_level" class="field"><option value="">—</option><option>特级</option><option>一级</option><option>二级</option><option>三级</option></select>
        </div>
      ...
      <div class="form-grid cols-2">
        ...
        <Field v-model="form.medical_history" label="既往病史" type="textarea" :rows="3" placeholder="高血压、糖尿病…" class="full" />
        <Field v-model="form.notes" label="备注" type="textarea" :rows="2" placeholder="其他注意事项" class="full" />
      </div>

      <div class="form-footer"><Btn variant="primary" :loading="loading" @click="submitPatient">保存档案</Btn></div>
    </GlassPanel>
  </div>
</template>

<style scoped>
.pa-view { max-width: 900px; }
.section-label { font: 600 var(--fz-sm, 13px)/1.4 var(--font-ui); color: var(--ink-2); margin: var(--sp-4, 16px) 0 var(--sp-2, 8px); text-transform: uppercase; letter-spacing: 0.04em; }
...
.form-footer { margin-top: var(--sp-5, 20px); display: flex; justify-content: flex-end; }
@media (max-width: 640px) { .form-grid.cols-3, .form-grid.cols-4 { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 480px) { .form-grid.cols-2, .form-grid.cols-3, .form-grid.cols-4 { grid-template-columns: 1fr; } }
</style>
