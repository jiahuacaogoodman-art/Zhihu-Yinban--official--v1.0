...
/**
 * UserManagement — 用户管理 + 角色权限
 *
 * 对应旧版 index.html 的 tab-users + tab-roles：
 *   - 创建用户 POST /api/auth/users
 *   - 列出用户 GET /api/auth/users
 *   - 签发 Token POST /api/auth/tokens
 *   - 查看角色 GET /api/auth/roles
 */
...
async function loadUsers() {
  ...
  try {
    ...
    if (e.status === 403 || e.status === 401) {
      toast({ tone: 'warning', text: '需要 admin 权限查看用户列表' })
    }
  } finally {
...
async function createUser() {
  ...
  try {
    ...
    toast({ tone: 'success', text: `用户 ${newUsername.value} 创建成功` })
    newUsername.value = ''
...
async function issueToken(userId: string, username: string) {
  ...
  try {
    ...
    const token = res.token ?? res.plain_token ?? ''
    alert(`Token 已签发，请立即保存（此后不再显示）：\n\n${token}`)
    toast({ tone: 'success', text: 'Token 已签发' })
  } catch (e: any) {
    toast({ tone: 'error', text: e.message ?? '签发失败' })
  }
}

function roleChipTone(key: string) {
  if (key === 'admin') return 'warning'
  if (key === 'nurse') return 'success'
  if (key === 'caregiver') return 'info'
  return 'accent'
}
...
<template>
  <div class="um-view">
    ...
    <GlassPanel variant="card">
      ...
      <Btn variant="primary" size="sm" :loading="creating" @click="createUser" style="margin-top: 12px;">
        创建用户
      </Btn>
      ...
      <div v-else class="um-list">
        <div v-for="u in users" :key="u.user_id" class="um-row">
          ...
          <Btn variant="ghost" size="sm" @click="issueToken(u.user_id, u.username)">
            签发 Token
          </Btn>
        </div>
      </div>
