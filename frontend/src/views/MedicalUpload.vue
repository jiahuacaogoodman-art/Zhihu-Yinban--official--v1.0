...
/**
 * MedicalUpload — 病历上传 OCR
 *
 * 对应旧版 index.html 的 tab-archive：
 *   - 选择已建档老人
 *   - 上传照片 → POST /api/ehr/records/upload (multipart)
 *   - 查看已上传病历列表 GET /api/ehr/records/{pid}
 */
...
async function upload() {
  if (!selectedPid.value) {
    toast({ tone: 'warning', text: '请先选择老人' })
    return
  }
  const files = fileInput.value?.files
  if (!files || files.length === 0) {
    toast({ tone: 'warning', text: '请选择病历照片' })
    return
  }
  ...
  try {
    ...
    toast({
      tone: 'success',
      text: hasWarn ? '照片已保存；部分文字未识别，可手动补充' : '病历和文字识别结果已保存',
    })
    // 重置
    ...
    toast({ tone: 'error', text: e.message ?? '上传失败' })
  } finally {
...
async function deleteRecord(docId: string) {
  if (!confirm('确定要删除这份病历？原图、识别文字都会被删除')) return
  try {
    await api.delete(`/ehr/records/${encodeURIComponent(docId)}`)
    toast({ tone: 'success', text: '病历已删除' })
    loadRecords()
  } catch (e: any) {
    toast({ tone: 'error', text: e.message ?? '删除失败' })
  }
}
...
<template>
  <div class="mu-view">
    <GlassPanel variant="card">
      <template #header>
        <span class="title-l">病历上传</span>
        <p class="meta">上传病历照片，系统会自动 OCR 识别文字并归入该老人档案</p>
      </template>

      <div class="form-grid cols-3">
        <div class="field-group">
          <span class="field-label">选择老人 *</span>
        ...
        <div class="field-group">
          <span class="field-label">病历类型</span>
          <select v-model="recordType" class="field">
      ...
      <Field
        ...
        type="textarea"
        :rows="3"
        placeholder="如果照片不清、OCR 不准，可在此手动补录关键信息"
      />
      <Field
        v-model="recordNotes"
        label="备注"
        type="textarea"
        :rows="2"
        placeholder="例如：2026-04-26 门诊，家属提供照片"
      />

      <div class="mu-footer">
        <Btn variant="primary" :loading="uploading" @click="upload">
          上传并识别文字
        </Btn>
      </div>
    </GlassPanel>
    ...
    <GlassPanel v-if="records.length > 0">
      <template #header>
        <span class="title-s">已上传病历（{{ records.length }}）</span>
      </template>
      <div class="mu-records">
        <div v-for="r in records" :key="r.doc_id" class="mu-record-card">
          ...
          <div class="mu-record-info">
            ...
            <Chip :tone="r.ocr_status === 'ocr_success' ? 'success' : 'warning'">
              {{ r.ocr_status === 'ocr_success' ? '已识别' : '待校对' }}
            </Chip>
            <p class="mu-ocr-text">{{ (r.ocr_text || r.manual_text || '未识别到文字').slice(0, 200) }}</p>
            <div class="mu-record-actions">
    ...
    <div v-else-if="selectedPid" class="empty">
      <p class="empty-title">该老人暂无病历照片</p>
    </div>
  </div>
</template>
...
@media (max-width: 640px) {
  ...
  .mu-record-card { grid-template-columns: 1fr; }
  .mu-thumb { width: 100%; height: 120px; }
}
</style>
