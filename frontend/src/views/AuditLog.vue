 ...
 * AuditLog — 审计日志
 *
 * 对应旧版 index.html 的 tab-audit：GET /api/ehr/audit
 */
...
async function loadAudit() {
  ...
  try {
    ...
    if (e.status === 403 || e.status === 401) {
      toast({ tone: 'warning', text: '需要 admin 权限查看审计日志' })
    } else {
      toast({ tone: 'error', text: e.message ?? '加载失败' })
    }
  } finally {
    loading.value = false
  }
...
<template>
  <div class="al-view">
    <GlassPanel variant="card">
      <template #header>
        <span class="title-l">审计日志</span>
        <p class="meta">全部档案写操作留痕（admin 可查）</p>
      </template>
      ...
      <div v-else class="al-list">
        <div v-for="(r, i) in records" :key="i" class="al-row">
          ...
          <div class="al-content">
            ...
            <span v-if="r.diff && Object.keys(r.diff).length" class="meta" style="color: var(--ink-4);">
              变更: {{ JSON.stringify(r.diff).slice(0, 120) }}
            </span>
          </div>
...
@media (max-width: 640px) {
  .al-filters { grid-template-columns: 1fr; }
